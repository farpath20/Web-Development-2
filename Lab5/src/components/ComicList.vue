<template>
  <div v-if="this.charactersList.length != 0">
    <router-link v-if="parseInt(this.$route.params.page)!=0" :to="{name: 'comicList', params: {page: parseInt(this.$route.params.page)-1}}">Prev</router-link>
    <br />
   <router-link :to="{name: 'comicList', params: {page: parseInt(this.$route.params.page)+1}}">Next</router-link>
  
     <br />

    <ul>
      <li v-for="(character,index) in charactersList" :key="index">
        <router-link :to="{name: 'comic', params: {id: character.id}}">{{character.title}}</router-link>
      </li>
    </ul>
  </div>
